...
export function parseSmcSearchHtml(html: string, term: string): SectionCandidate[] {
...
export class SmcProvider implements AvailabilityProvider {
  ...
  async searchSections(params: { term: string; q?: string; subject?: string; number?: string; limit?: number }) {
    ...
    const results = parseSmcSearchHtml(html, params.term).map((item) => ({
      ...item,
      externalUrl: url
    }));
