...
export function parseIvcSearchHtml(html: string): SectionCandidate[] {
...
function buildSearchUrl(query: SearchQuery) {
  const base = env.IVC_SEARCH_BASE_URL.replace(/\/$/, '');
  ...
  return queryString ? `${base}/ClassSearchForm.aspx?${queryString}` : `${base}/ClassSearchForm.aspx`;
}
...
export class IvcProvider implements AvailabilityProvider {
  async searchSections(query: SearchQuery): Promise<SectionCandidate[]> {
    ...
    const results = parseIvcSearchHtml(html);
  ...
  async getAvailability(input: AvailabilityInput): Promise<AvailabilityResult> {
    const url = input.detailUrl || buildSearchUrl({ collegeSlug: 'ivc' } as SearchQuery);
    const response = await fetchWithTimeout(url);
    if (!response.ok) {
